...
export default function ProjectDetail() {
  ...
  const getGalleryPlaceholder = (index: number, projectTitle: string) => {
    const configs = [
      ...
      { bg: 'linear-gradient(135deg, #3b82f6 0%, #1e40af 100%)', icon: '⚙️', title: 'Settings', subtitle: 'Configuration' },
      { bg: 'linear-gradient(135deg, #06b6d4 0%, #3b82f6 100%)', icon: '📈', title: 'Analytics', subtitle: 'Performance Data' },
      { bg: 'linear-gradient(135deg, #8b5cf6 0%, #06b6d4 100%)', icon: '💳', title: 'Payments', subtitle: 'Transaction System' },
    ];
  ...
  // Final screens placeholder configurations
  const getFinalScreenPlaceholder = (index: number, projectTitle: string) => {
    const configs = [
      { bg: 'linear-gradient(135deg, #06b6d4 0%, #8b5cf6 100%)', icon: '🏁', title: 'Final Design', subtitle: 'Completed Interface' },
      { bg: 'linear-gradient(135deg, #10b981 0%, #06b6d4 100%)', icon: '✨', title: 'Polished UI', subtitle: 'Ready for Launch' },
      { bg: 'linear-gradient(135deg, #f97316 0%, #ef4444 100%)', icon: '🚀', title: 'Production', subtitle: 'Live Application' },
      { bg: 'linear-gradient(135deg, #8b5cf6 0%, #ec4899 100%)', icon: '🎨', title: 'Visual Design', subtitle: 'Final Screens' },
      { bg: 'linear-gradient(135deg, #3b82f6 0%, #06b6d4 100%)', icon: '💎', title: 'Premium UI', subtitle: 'High Fidelity' },
    ];
    ...
    return `
      ...
      ">
        <div>
          <div style="font-size: 2.5rem; margin-bottom: 0.75rem;">${config.icon}</div>
          <div style="font-size: 1.25rem; font-weight: 600; margin-bottom: 0.5rem;">${config.title}</div>
          <div style="font-size: 0.85rem; opacity: 0.8; margin-bottom: 0.75rem;">${config.subtitle}</div>
          <div style="font-size: 0.7rem; opacity: 0.6;">${projectTitle}</div>
        </div>
      </div>
    `;
  };
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="rounded-2xl overflow-hidden mb-12">
            <img
              ...
              onError={(e) => {
                e.currentTarget.style.background = 'linear-gradient(135deg, #0891b2 0%, #1e40af 100%)'
                e.currentTarget.style.display = 'flex'
                e.currentTarget.style.alignItems = 'center'
                e.currentTarget.style.justifyContent = 'center'
                e.currentTarget.innerHTML = '<div class="text-white text-xl font-semibold">Project Cover</div>'
              }}
            />
          </div>
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="grid lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2 space-y-8">
              ...
              <div>
                <h3 className="text-2xl font-semibold mb-4 text-[#fff]">The Challenge</h3>
                <p className="text-slate-300 leading-relaxed">{project.challenge}</p>
              </div>

              <div>
                <h3 className="text-2xl font-semibold mb-4 text-[#fff]">The Solution</h3>
                <p className="text-slate-300 leading-relaxed">{project.solution}</p>
              </div>

              <div>
                <h3 className="text-2xl font-semibold mb-4 text-[#fff]">The Outcome</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {project.gallery.map((image, index) => (
                <div key={index} className="rounded-xl overflow-hidden bg-slate-700/50 shadow-lg hover:shadow-xl transition-shadow duration-300 relative group">
                  ...
                  {/* Original image as overlay that will hide the placeholder when loaded */}
                  <img
                    src={image}
                    alt={`${project.title} - Image ${index + 1}`}
                    className="absolute inset-0 w-full h-full object-cover hover:scale-105 transition-transform duration-300 opacity-0"
                    onLoad={() => {
                      console.log(`Successfully loaded image: ${image}`)
                      // Only show the actual image if it's different from cover
                      // For now, we'll keep showing placeholders as they're more informative
                    }}
                    onError={() => {
                      console.log(`Failed to load image: ${image}`)
                      // Placeholder is already shown, just log the error
                    }}
                  />
                ...
                <div key={index} className="rounded-xl overflow-hidden shadow-lg">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.currentTarget.style.alignItems = 'center'
                      e.currentTarget.style.justifyContent = 'center'
                      e.currentTarget.innerHTML = '<div class="text-white font-semibold">Final Screen</div>'
                    }}
                  />
                </div>
              ))}
